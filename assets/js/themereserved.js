...
$('.dates span:not(.disable)').click(function (e) {
    ...
    if (old_input.length === 0) {
        var input = '<input type="hidden" name="CarOrderForm[reserved_dates][]" value="' + $(this).data("date") + '">';
        form.append(input);
    } else {
        old_input.remove();
    }
...
$('.payments-options a').click(function () {
    var p_type = $(this).data('type');
    $('form.form-extras').find('input[name="CarOrderForm[payment_type]"]').val(p_type);
});
    ...
    if (typeof google === 'object' && typeof google.maps === 'object') {
        if ($('#map-canvas').length) {
            ...
            var mapIWcontent = '' +
                ...
                '<div class="caption-text">Start from 89$/per a day</div>' +
                '<div class="buttons">' +
        ...
        if ($('#map-canvas-fs').length) {
            ...
            google.maps.event.addDomListener(window, 'load', function () {
                ...
                marker = new google.maps.Marker({
                    ...
                    map: map,
                    icon: image,
                    title: 'Hello Worldqwdq!'
                });
                marker2 = new google.maps.Marker({
                    position: new google.maps.LatLng(41.007135, 28.910556), // marker coordinates
                    map: map,
                    icon: image,
                    title: 'Hello Worldqwd!'
                });
                marker3 = new google.maps.Marker({
                    position: new google.maps.LatLng(41.040807, 28.848071), // marker coordinates
                    map: map,
                ...
                marker4 = new google.maps.Marker({
                    ...
                    map: map,
                    icon: image,
                    title: 'Hello World!wqww'
                });
                marker5 = new google.maps.Marker({
                    position: new google.maps.LatLng(41.077050, 28.995013), // marker coordinates
                    map: map,
                    icon: image,
                    title: 'Hello Worldwew!'
                });
    ...
    if ($.ui) {
        if ($(priceSliderRange).length) {
            $(priceSliderRange).slider({
                ...
                min: 0,
                max: 2000,
                values: [$('#amount-from').val(), $('#amount-to').val() > 0 ? $('#amount-to').val() : 2000],
